    ...
    setTimeout(() => {
      if (mode === 'login') {
        ...
        if (db.register(email, password)) {
          setSuccess('Conta Alpha criada! Entre com seus dados.');
          setMode('login');
          setIsLoading(false);
        } else {
        ...
        db.addRecoveryRequest(email);
        setSuccess('Solicitação enviada! O administrador revisará seu pedido em breve.');
    ...
    <div className="min-h-screen flex items-center justify-center alpha-gradient p-4 relative overflow-hidden">
      ...
      <div className="bg-white w-full max-w-[420px] rounded-[3rem] shadow-2xl overflow-hidden relative z-10 animate-in fade-in zoom-in duration-700 border border-white/20">
        <div className="p-10 text-center">
          ...
          <p className="text-[10px] uppercase tracking-[0.4em] font-black text-cyan-600 mt-1">
            {mode === 'recover' ? 'Recuperação de Acesso' : 'Consultório Digital'}
          </p>
        </div>

        <div className="px-10 pb-12">
          {success ? (
             <div className="bg-emerald-50 border border-emerald-100 p-6 rounded-3xl text-center space-y-3 mb-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              >
                {isLoading ? (
                  ...
                  mode === 'login' ? 'Entrar no Sistema' : mode === 'register' ? 'Criar minha Conta' : 'Solicitar Nova Senha'
                )}
              </button>
            </form>
          )}
          ...
          <div className="mt-8 pt-6 border-t border-slate-50 flex flex-col gap-4 items-center">
            {mode === 'login' ? (
              <p className="text-xs text-slate-400 font-bold">
                Novo no Alpha Wolves?
                <button 
                  onClick={() => { setMode('register'); setError(''); }}
                  className="ml-2 text-blue-700 font-black uppercase hover:underline"
                >
                  Cadastrar-se
                </button>
              </p>
              ...
              <button 
                ...
                className="text-xs text-blue-700 font-black uppercase hover:underline"
              >
                Voltar para Login
              </button>
            )}
