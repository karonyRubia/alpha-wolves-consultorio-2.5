                ...
                <section className="space-y-4 no-print">
                  <div className="flex items-center justify-between">
                    ...
                    <button 
                      ...
                      className="bg-emerald-50 text-emerald-600 px-4 py-2 rounded-xl text-[10px] font-black uppercase border border-emerald-100 hover:bg-emerald-100 transition-all flex items-center gap-2"
                    >
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                    {selectedPatient.files?.map(file => (
                      <div key={file.id} className="group relative bg-slate-50 border border-slate-100 rounded-2xl overflow-hidden hover:shadow-md transition-all">
                        {file.type.startsWith('image/') ? (
                          <img src={file.data} className="w-full h-32 object-cover" alt={file.name} />
                        ) : (
                          <div className="w-full h-32 flex flex-col items-center justify-center bg-blue-50">
                            <svg className="w-10 h-10 text-blue-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
                            <span className="text-[8px] font-black text-blue-600 uppercase mt-2">PDF Documento</span>
                          </div>
                        )}
                        <div className="p-2">
                          <p className="text-[9px] font-bold text-slate-600 truncate">{file.name}</p>
                          <p className="text-[7px] text-slate-400 uppercase">{new Date(file.date).toLocaleDateString()}</p>
                        </div>
                    ...
                    {(!selectedPatient.files || selectedPatient.files.length === 0) && (
                      <div className="col-span-full py-8 border-2 border-dashed border-slate-100 rounded-3xl flex flex-col items-center justify-center text-slate-300">
                        <svg className="w-8 h-8 mb-2" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a2 2 0 002 2h12a2 2 0 002-2v-1m-4-8l-4-4m0 0L8 8m4-4v12" /></svg>
